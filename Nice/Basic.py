...
def power(box_code, box_dash):
    ...
    if need_to_tidy_up:
        ...
        powered_denominator = ""
        powered_numerator = powers.power_distributor(f"({nice_box}{shift})", products.return_number(power-1))
...
def sin_cos_tan(box_code, box_dash):
    ...
    return constant_product, box_v, box_c, shift, box_dash_v, power, nice_box


# sin [has function_determiner value = 1]
def sin(box_code, box_dash):
    constant_product, box_v, box_c, shift,box_dash_v,power,nice_box = sin_cos_tan(box_code, box_dash)
    # in the case that box_dash is a fraction
    box_dash_v_numerator, box_dash_v_denominator = splitter(box_dash_v)
...
def cos(box_code, box_dash):
    ...
    # in the case that box_dash is a fraction
    box_dash_v_numerator, box_dash_v_denominator = splitter(box_dash_v)
    cosine = powers.power_distributor(f"cos({nice_box}{shift})", products.return_number(power-1))
    sin = f"sin({nice_box}{shift})"
    numerator = products.multiply_two_together(sin, cosine, False)
    numerator = products.multiply_two_together(numerator, box_dash_v_numerator, False)
    numerator, denominator = quotients.divide(numerator, box_dash_v_denominator, False)
    return [quotients.assembler(numerator,denominator), products.return_number(-constant_product)]
...
def tan(box_code, box_dash):
    ...
    # in the case that box_dash is a fraction
    box_dash_v_numerator, box_dash_v_denominator = splitter(box_dash_v)
    tan = powers.power_distributor(f"tan({nice_box}{shift})", products.return_number(power-1))
    sec = f"cos({nice_box}{shift})^-2 "
